...
return defaultValue;};provider.$get=function(){return provider;};return provider;}]);angular.module('website.core.widgets',['website.core.animation','website.core.entertainment']);angular.module('website',['core.library.config','core.library.jsonrpc','website.core','website.widgets','website.plugins','website.moto_link','moto.validation','ngStorage','ipCookie']).config(['$compileProvider','$httpProvider','$localStorageProvider',function($compileProvider,$httpProvider,$localStorageProvider){$compileProvider.debugInfoEnabled(false);$httpProvider.useApplyAsync(true);$localStorageProvider.setDeserializer(function(value){try{if(angular.isString(value)&&value.length&&(value[0]==='{'||value[0]==='[')){return angular.fromJson(value);}
...
this.registerCallback=function(callbackFn){if(!angular.isFunction(callbackFn)||(callbacks.indexOf(callbackFn)>-1))return angular.noop;callbacks.push(callbackFn);return function(){callbacks[callbacks.indexOf(callbackFn)]=null;};};this.start=function(){interval=$interval(processCallbacks,intervalTimer);};this.stop=function(){$interval.cancel(interval);};this.start();}]);angular.module('website.moto_link').directive('motoInitLightboxGallery',function(){var DEFAULT_PROPERTIES={itemRootSelector:'[data-moto-lightbox-item]',itemLinkSelector:'[data-moto-lightbox-link]',itemCaptionSelector:'[data-moto-lightbox-caption]'};return{restrict:'A',link:function($scope,$element,$attrs){$element.magnificPopup({delegate:'a[data-action=lightbox]'+DEFAULT_PROPERTIES.itemLinkSelector,type:'image',tClose:'',tLoading:'',mainClass:angular.isDefined($attrs.tileGalleryShowCounter)?'':'moto-lightbox_hidden-counter',gallery:{enabled:true,preload:[5,10],tPrev:'',tNext:'',tCounter:'%curr% / %total%'},image:{titleSrc:function(item){return angular.element(item.el.context).closest(DEFAULT_PROPERTIES.itemRootSelector).find(DEFAULT_PROPERTIES.itemCaptionSelector).html()||'';}},zoom:{enabled:true,duration:400,easing:'ease-in-out'}});}};});angular.module('website.moto_link').run(['$timeout',function($timeout){function openInLightBox(event){var $element=$(event.currentTarget);if(!$element.closest('[data-moto-lightbox-caption]').length&&$element.closest('[data-moto-init-lightbox-gallery]').length){return;}
...
if(needCheckOnResize){object.$pseudoElement.hide();object.$element.removeClass(settings.attachedClass).removeClass(settings.attachedClass+'_'+object.options.direction).css('width','').css('marginTop','');elementWidth=object.$element.innerWidth();object.$pseudoElement.show();object.$element.innerWidth(elementWidth);object.$pseudoElement.innerWidth(elementWidth);object.$element.addClass(settings.attachedClass).addClass(settings.attachedClass+'_'+object.options.direction);}}catch(Error){isDebug()&&console.info('motoSticky : ERROR on syncPseudoElement',Error);}}
function attachObject(object){if(!object.isAttached){object.$element.show().addClass(settings.attachedClass).addClass(settings.attachedClass+'_'+object.options.direction);object.isAttached=true;}
...
object={$scope:$scope,$element:$element,$attrs:$attrs,options:angular.extend({},defaultOptions,options),isAttached:false};addLazyElementsLoadListeners(object);initPseudoElement(object);checkObject(object);objects.push(object);}catch(Error){isDebug()&&console.info('motoSticky : ERROR on addObject',Error);}}
...
instance=$element.bxSlider(settings);$scope.$on('$destroy',instance.destroySlider);Entertainment.$onLetsDanceEvent(entertainmentEnv,function(){instance.redrawSlider();if(Entertainment.isDisabledPlaying($element)){return;}
if(properties.slideshowEnabled){instance.startAuto();}
$timeout(instance.redrawSlider);});Entertainment.$onLetsStopEvent(entertainmentEnv,function(){instance.stopAuto();});}};}]);angular.module('website.widget.contact_form',['core.library.jsonrpc','ngFileUpload']).service('widget.ContactForm.Service',['jsonrpc',function(jsonrpc){var service=jsonrpc.newService('Widget.ContactForm');this.sendMessage=service.createMethod('sendMessage');this.getApiPath=jsonrpc.getBasePath;}]).directive('motoWidgetContactForm',['$timeout',function($timeout){return{restrict:'C',scope:true,compile:function($element){return{pre:function($scope){if($element.closest('.moto-widget_interactive').length<1){return;}
...
$attrs.onExpiry&&$attrs.onExpiry!=='stop'&&$scope.$on('timer-stopped',onExpiryHandler);}};}};}]);angular.module('website.widget.disqus',['website.core']).directive('motoWidgetDisqus',['motoDependencyService',function(DependencyService){var exists=false;return{restrict:'AC',link:function($scope,$element,$attrs){var params;try{if(exists){return $element.remove();}
...
return{restrict:'A',priority:450,link:function($scope,$element,$attrs){var entertainmentEnv={$scope:$scope,$element:$element};var properties=$scope.$eval($attrs.motoSliderOptions);var settings=getSliderSettings(properties);var instance;settings.onSliderLoad=function(){$element.closest('.moto-widget-slider').removeClass('moto-widget-slider-loader');};instance=$element.bxSlider(settings);$scope.$on('$destroy',instance.destroySlider);Entertainment.$onLetsDanceEvent(entertainmentEnv,function(){instance.redrawSlider();if(Entertainment.isDisabledPlaying($element)){return;}
if(properties.slideshowEnabled){instance.startAuto();}
...
heightWatcherInstance();WidgetsRepository.forget(thisWidget);});}};}};}]);angular.module('website.core.form').directive('motoFormAttachmentButton',['$timeout',function($timeout){function updateModelValidity(model,errors,state){var i;var len;if(angular.isUndefined(errors)||angular.isUndefined(state)){return false;}
...
$scope.$on('$destroy',function(){if(unWatch){unWatch();unWatch=null;}});}};}]);angular.module('website.widgets').service('website.library.PaginatorClass',[function(){function PageItemClass(value){this.value=value;this.number=null;if(angular.isObject(value)&&angular.isDefined(value['number'])){this.number=value.number;}}
PageItemClass.prototype={value:null,number:null,getLink:function(){return this.value&&this.value.link;}};function transformItem(item){if(item instanceof PageItemClass){return item;}
return new PageItemClass(item);}
function transformItems(items){var result=[];var i;var len;for(i=0,len=items.length;i<len;i++){result.push(transformItem(items[i]));}
return result;}
...
this.pages=transformItems(data.pages);this.pageCount=data.pages.length;this.firstNumber=1;this.first=this.getFirstPage()||new PageItemClass();this.last=this.getLastPage()||new PageItemClass();this.lastNumber=this.last.number;this.setCurrentPageNumber(data.current);return true;},getPageByNumber:function(number){return this.pages[number-1];},getFirstPage:function(){return this.pages[0];},getPreviousPage:function(){return this.getPageByNumber(this.currentNumber-1);},getCurrentPage:function(){return this.getPageByNumber(this.currentNumber);},getNextPage:function(){return this.getPageByNumber(this.currentNumber+1);},getLastPage:function(){return this.pages[this.pages.length-1];},setCurrentPageNumber:function(number){number=parseInt(number);if(isNaN(number)){return false;}
this.currentNumber=number;if(number>1){this.previousNumber=number-1;}
if(number<this.pages.length){this.nextNumber=number+1;}
this.previous=this.getPreviousPage()||new PageItemClass();this.current=this.getCurrentPage()||new PageItemClass();this.next=this.getNextPage()||new PageItemClass();this.updatePagesInRange();return true;},selectPage:function(page){if(angular.isNumber(page)){page=this.getPageByNumber(page);}
if(page instanceof PageItemClass){this.$trigger('selected',page.value);}}};return PaginatorClass;}]);(function($){var ITEMS_SELECTOR='>.moto-widget-tabs__wrapper>.moto-widget-tabs__items-wrapper>.moto-widget-tabs__item';var ITEMS_DESKTOP_HEADER_SELECTOR='>.moto-widget-tabs__wrapper>.moto-widget-tabs__headers-wrapper>.moto-widget-tabs__header';var ITEMS_MOBILE_HEADER_SELECTOR=ITEMS_SELECTOR+'>.moto-widget-tabs__header';var ITEMS_HEADERS_SELECTOR=ITEMS_DESKTOP_HEADER_SELECTOR+','+ITEMS_MOBILE_HEADER_SELECTOR;var HEADER_OPENED_CLASS='moto-widget-tabs__header_opened';var ITEMS_DESKTOP_OPENED_HEADER_SELECTOR=ITEMS_DESKTOP_HEADER_SELECTOR+'.'+HEADER_OPENED_CLASS;var _$window=$(window);function isMobileDevice(){return(window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth)<1040;}
